      ...
      (let* ((line-start-pos (point))
        ...
        (when (seed7-line-code-ends-with 0 ");")
          ...
          (when (seed7-re-search-backward ");" line-start-pos)
            ...
            ;; closing paren; `backward-sexp' needs point *after* it to
            ;; jump to the beginning of the whole (possibly deeply nested) array literal.
...
(defun seed7-complete-statement-or-indent ()
  "Expand keyword or adjust indentation of current line or marked block.

If a block is marked, indent it, regardless of point position inside the
marked area.
               ...
               (or (and (or in-indent
                   ...
                   (and (looking-at-p " ?)")
                        (member keyword '("in"
                                          ...
                                          "val"
                                          "callbn")))))
          (let ((overwrite-mode (if seed7-template-expansion-disables-overwrite-mode
                                   nil
                                 overwrite-mode)))
            (backward-word)
            (cond
