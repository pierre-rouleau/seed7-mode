...
;; Time-stamp: <2026-06-30 19:30:39 EDT, updated by Pierre Rouleau>
...
(defun sd7-indent-perf--profiler-entry-name (entry)
  "Return a readable name string for a profiler calltree ENTRY.
ENTRY is normally a function symbol, but for closures, lambdas, or
byte-compiled anonymous functions it may be a list/compiled-function
object instead, so fall back to `prin1-to-string' in that case."
  (cond
   ((symbolp entry) (symbol-name entry))
   ((and (consp entry) (symbolp (car entry)))
    ;; e.g. (closure ...) or (lambda ...): show a short tag instead of
    ;; dumping the whole closure body.
    (format "#<%s>" (car entry)))
...
count.  Works in both interactive and batch mode."
  (let ((log (profiler-cpu-log)))
    (if (null log)
      ...
      (let ((tree    (profiler-calltree-build log))
            (entries '()))
            ...
            (dolist (e entries)
              (insert (format "%-8d  %5.1f%%  %s\n"
                              ...
                              (if (> total 0)
                                ...
                                0.0)
                              (sd7-indent-perf--profiler-entry-name (cdr e)))))
            (buffer-string)))))))
